...
def DynamicConv(input_size, kernel_size=3, padding_l=None, num_heads=1,
    ...
    if torch.cuda.is_available():
        try:
            from models.modules.dynamicconv_layer import DynamicconvLayer
...
class DynamicConv1dTBC(nn.Module):
    ...
    def _forward_expanded(self, x, incremental_stat, query):
        ...
        if self.weight_softmax and self.renorm_padding:
            ...
            weight_expanded = weight_expanded.narrow(2, self.padding_l-1, T)
            ...
            weight_expanded.as_strided((B*H, T, K), (T*(T+K-1), T+K, 1)).copy_(weight)
            weight_expanded = weight_expanded.narrow(2, P-1, T)  # B*H x T x T
